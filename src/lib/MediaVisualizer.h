...
private:
  ...
  String mediaStatus;
  bool isPlaying;
  bool hasValidMetadata; // Flag untuk cek apakah ada metadata valid
  ...
  // Helper function untuk cek apakah metadata valid
  ...
  void drawMetadata()
  {
    // Hanya tampilkan metadata jika ada data valid
    if (!hasValidMetadata)
      return;
  ...
  {
    // Hanya scroll jika ada metadata valid
    if (!hasValidMetadata)
      return;
...
public:
  ...
  void handleMediaData(JsonDocument &doc)
  {
    ...
    if (strcmp(type, "media") == 0)
    {
      ...
      // Update flag metadata valid
      hasValidMetadata = checkValidMetadata();

      // Reset scroll positions hanya saat media berubah
      if (titleChanged)
      {
        titleScrollPos = 0;
      }
      ...
      // Aktifkan visualizer jika ada amplitude ATAU metadata valid
      if (hasAmplitude || hasValidMetadata)
      {
        isActive = true;
      }

      // Log hanya saat ada perubahan signifikan
  ...
  void update()
  {
    ...
    if (now - lastUpdateTime >= updateInterval)
    {
      ...
      // Tampilkan metadata hanya jika valid
      if (hasValidMetadata)
      {
        drawMetadata();
        // Draw separator line
  ...
  // Manual trigger untuk visualizer only mode (tanpa metadata)
